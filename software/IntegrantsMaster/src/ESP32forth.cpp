...
/*
 * ESP32forth v7.0.7.16
 * Revision: 0a102d9785e1ec0b8397
 */
...
#define VOCABULARY_LIST \
  ...
  V(ledc) V(http) V(Wire) V(WiFi) V(sockets) \
...
#else
#warning serial 2 enabled
 # define OPTIONAL_SERIAL2_SUPPORT \
  XV(serial, "Serial2.begin", SERIAL2_BEGIN, Serial2.begin(tos, SERIAL_8N1, 22, 27); DROP) \
...
: free. ( nf nu -- ) 2dup swap . ." free + " . ." used = " 2dup + . ." total ("
                     over + 100 -rot */ n. ." % free)" ;
: raw-ok   ."  v7.0.7.16 - rev 0a102d9785e1ec0b8397" cr
