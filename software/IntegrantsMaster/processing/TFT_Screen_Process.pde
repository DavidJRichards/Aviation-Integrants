...
// djrm modified for integrants
// enable button needs to be pressed for each capture after initial
...
int serial_baud_rate = 115200; //  was 921600                                                #
//                                                                                           #
...
void draw() {
  ...
  case 6: // Fade the old image if enabled
    if ( fadedImage() == true )
    {
      state = 0; // Go to next state when image has faded
    }
    else
		{
			// force single capture unless in fade mode
      running=false;
		}
    break;
